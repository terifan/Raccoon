 ...
 *   4 bytes - offset
 */
public class ArrayMap2 implements Iterable<ArrayMapEntry2>
{
	 ...
	 * @return the buffer
	 */
	public ArrayMap2(int aCapacity)
	{
	 ...
	 * @return the buffer
	 */
	public ArrayMap2(byte[] aBuffer, int aOffset, int aCapacity)
	{
	...
	public boolean put(ArrayMapEntry2 aEntry, Result<ArrayMapEntry2> oExistingEntry)
	{
		...
		{
			throw new IllegalArgumentException("Entry length exceeds capacity of this map: " + (key.length + valueLength) + " > " + (mCapacity - ENTRY_HEADER_SIZE - HEADER_SIZE - ENTRY_POINTER_SIZE));
		}
		...
		else if (getFreeSpace() < ENTRY_HEADER_SIZE + key.length + valueLength + ENTRY_POINTER_SIZE)
		{
			return false;
		}
		else
	...
	public boolean get(ArrayMapEntry2 aEntry)
	{
	 ...
	 */
	public int nearest(ArrayMapEntry2 aEntry)
	{
		int index = indexOf(aEntry.getKey());
	...
	public boolean remove(ArrayMapEntry2 aEntry, Result<ArrayMapEntry2> oOldEntry)
	{
		int index = indexOf(aEntry.getKey());
	...
	private void removeImpl(int aIndex, Result<ArrayMapEntry2> oOldEntry)
	{
	...
	@Override
	public Iterator<ArrayMapEntry2> iterator()
	{
		return new EntryIterator();
	}


	public class EntryIterator implements Iterator<ArrayMapEntry2>
	{
		...
		public ArrayMapEntry2 next()
		{
			...
			ArrayMapEntry2 entry = new ArrayMapEntry2();
	...
	public ArrayMapEntry2 getFirst()
	{
		ArrayMapEntry2 entry = new ArrayMapEntry2();
		get(0, entry);
		return entry;
	...
	public ArrayMapEntry2 removeFirst()
	{
		Result<ArrayMapEntry2> entry = new Result<>();
		removeImpl(0, entry);
		return entry.get();
	}


	public ArrayMapEntry2 removeLast()
	{
		Result<ArrayMapEntry2> entry = new Result<>();
	...
	public ArrayMap2 resize(int aNewSize)
	{
