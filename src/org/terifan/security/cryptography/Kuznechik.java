...
public final class Kuznechik implements BlockCipher
{
	...
	private static void kuz_l_fast(int[] w)
	{
	...
	private static void kuz_l(byte[] w)
	{
		for (int j = 0; j < 16; j++)
		{
			byte x = w[15];
	...
	private static void kuz_l_inv(byte[] w)
	{
		for (int j = 0; j < 16; j++)
		{
			byte x = w[0];
	...
	private static byte kuz_mul_gf256_fast(byte a, byte b)
	{
		if (a == 0 || b == 0)
		{
			return 0;
	...
	public Kuznechik(SecretKey aKey)
	{
		engineInit(aKey);
	}
