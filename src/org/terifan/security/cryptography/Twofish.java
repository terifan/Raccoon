 ...
 *
 * In each round, a 64-bit S-box value is computed from 64 bits of the block, and this value is xored into the other half of the block. The
 * two half-blocks are then exchanged, and the next round begins. Before the first round, all input bits are xored with key-dependent
 * "whitening" subkeys, and after the final round the output bits are xored with other key-dependent whitening subkeys; these subkeys are
