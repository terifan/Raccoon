...
public final class MD5 extends MessageDigest implements Cloneable
{
	...
	private transient int[] transformBuffer;
	private transient byte[] digestBits;
	private transient byte[] finalDigest;
	private transient boolean hasDigest;
	...
	protected byte[] engineDigest()
	{
		...
		{
			byte[] r = new byte[16];
			System.arraycopy(finalDigest, 0, r, 0, 16);
			return r;
		}
		...
		System.arraycopy(digestBits, 0, dummy, 0, 16);

		engineReset();

		System.arraycopy(dummy, 0, finalDigest, 0, 16);
		hasDigest = true;
	...
	public MD5 clone()
	{
		...
		instance.digestBits = this.digestBits.clone();
		instance.finalDigest = this.finalDigest.clone();
		instance.hasDigest = this.hasDigest;
		instance.state = this.state.clone();
		instance.transformBuffer = this.transformBuffer.clone();
